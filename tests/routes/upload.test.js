...
test('uploading a .zip on an assignment with a pending submission replaces the files in place', async () => {
  ...
  const submission = db.createSubmission({ assignmentId: assignment.id, studentId: 'student-1', files: ['solution.zip'] });
  const app = createApp();
  const agent = await loginAs(app, 'student-1');

  const before = db.getSubmissions().find((s) => s.id === submission.id);
...
test('uploading after a not_done verdict creates a new chained submission', async () => {
  ...
  // student-2 has never submitted this assignment yet; mark a fresh not_done state by
  // creating a rejected submission directly, then resubmit as the student would.
  const rejected = db.createSubmission({ assignmentId: assignment.id, studentId: 'student-2', files: ['bad.zip'] });
  ...
  assert.strictEqual(res.status, 302);
  const submissions = db.getSubmissions().filter((s) => s.assignmentId === assignment.id && s.studentId === 'student-2');
...
test('blocks upload once the due date has passed and nothing was ever submitted', async () => {
  ...
  db.setAssignmentDueDateForTests(assignment.id, '2000-01-01');
...
test('student cannot submit for an assignment they cannot see', async () => {
  const individual = seedAssignment({
    title: 'Персональное',
    mdPath: 'individual/student-1/x.md',
  ...
  const res = await agent
    .post(`/assignment/${individual.id}/submit`)
